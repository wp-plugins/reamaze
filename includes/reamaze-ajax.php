  ...
  public static function convert_to_conversation() {
    ...
    if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
      ...
      $conversation = array(
        ...
        "category" => $_POST['category'], // TODO: let user choose
        "message" => array(
      ...
      if ( $result && ! empty( $result['slug'] ) ) {
        ...
        if ( ! empty( $_POST['include_reply'] ) && ! empty( $_POST['reply_message'] ) ) {
          $message = Reamaze\API\Message::create( array( "conversation_slug" => $result['slug'], "body" => $_POST['reply_message'], "visibility" => 0 ) );
          ...
          if ( !empty( $_POST['add_wp_reply'] ) ) {
            ...
            $wpReplyID = wp_new_comment( array(
              ...
              'comment_content' => $parsedown->text($_POST['reply_message']),
        ...
        if ( ! empty( $_POST['add_note'] ) && ! empty( $_POST['note_message'] ) ) {
          $internal_note = Reamaze\API\Message::create( array( "conversation_slug" => $result['slug'], "body" => $_POST['note_message'], "visibility" => 1 ) );
        }
